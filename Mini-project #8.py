...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        ...
        if dist(rock_pos, my_ship.pos) > (40 + my_ship.get_radius()):
...
# process group - object collision
...
# process group - group colission
def group_group_collide(gpr, gpm):
     
    counter = 0
    
    for element in gpm:
    
        if group_collide(gpr, element):
            counter += 1
            gpm.discard(element)
...
# initialize ship, rocks, missiles, explosions sprite sets
